...
function checkNoLinks() {
  ...
  var arr  = [
    { content: 'This', title: 'There is no linked element: xx' },
    { content: '(No link)', title: 'There is no linked element: xx' },
  ];
  for (var i = 0, n = nolinks.length; i < n; i++) {
    assertEqual('Content of nolink', nolinks[i].textContent, arr[i].content);
    assertEqual('Title of nolink', nolinks[i].title, arr[i].title);
  }
}
