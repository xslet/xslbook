...
<xsl:stylesheet version="1.0"
 ...
 <xsl:template match="/book">
  <html>
   <head>
    ...
    <xsl:call-template name="bk:load_default_script"/>
    <xsl:call-template name="bk:load_default_css"/>
   </head>
   <body>
    <main>
     <header>
 ...
 <xsl:template name="bk:load_default_script">
  <script src="{concat($bk:xsl_dir, '/xslbook.js')}"></script>
 </xsl:template>

 <xsl:template name="bk:load_default_css">
